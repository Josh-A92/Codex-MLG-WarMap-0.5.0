...
| Resource definitions (season systems) | Official FAQ provenance confirms Season 2 introduces Red Copper and Holy Water as season-resource context; Season 1 Coal/Mithril are not converted into those two during transition. | confirmed_official | Official_Artwork/S2_Facts_01.png, X-Clash_Season_2_Asset_Register.xlsx (S2-011 official X-Clash Facebook provenance) | rulesDefinition.resourceModel.metadata/context |
...
| resource-model architecture decision | Official evidence confirms Red Copper and Holy Water as Season 2 resource context, while the current canonical resourceModel shape centers on one primary resourceId. Decide whether to select one strategic-map resource or evolve the package contract to support multiple resource definitions. | Required to avoid forcing placeholders or incorrect semantics in Season 2 resource/scoring modeling. | No (if a structurally valid model is chosen) | Yes |
...
- Principal blocker is the unresolved Season 2 resource-model boundary (single-resource selection vs contract evolution for multiple resource definitions) plus missing authoritative map-output/scoring relationships.
...
1. Resource-model architecture decision before package authoring: choose one strategic-map resource for current contract shape or evolve the package contract to support multiple Season 2 resources (Red Copper and Holy Water context is officially confirmed).
2. Official or in-game authoritative rule table for map-output/scoring relationships (structure outputs, modifiers, and scoring formulas) before any calculated Season 2 totals are published.
3. Authoritative evidence for advanced capture/unlock/phase/buff behavior where non-empty entries are expected; otherwise explicit product sign-off that empty containers are semantically intentional.
4. Product/runtime configuration decisions for runnable publication: data-source paths, workspace values, participating-server scope, and chosen package seasonStatus for this release.
